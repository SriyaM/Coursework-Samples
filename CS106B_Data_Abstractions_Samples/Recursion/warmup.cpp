...
STUDENT_TEST("myPower(), test on negative 3") {
    EXPECT_ERROR(factorial(-3));
}


STUDENT_TEST("myPower(), test on large range with negatives") {
...
PROVIDED_TEST("myPower(), to narrow down which bases and exponent combinations fail") {
    EXPECT_EQUAL(myPower(5, -2), pow(5, -2));
    EXPECT_EQUAL(myPower(6, -7), pow(6, -7));
    EXPECT_EQUAL(myPower(-10, 10), pow(-10, 10));
    EXPECT_EQUAL(myPower(-5, -1), pow(-5, -1));
}
