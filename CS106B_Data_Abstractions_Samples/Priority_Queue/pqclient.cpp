...
/*
 * This function takes in a vector of Datapoints and uses the PQArray class to
 * sort the DataPoints in increasing order based on their priority.
 */
...
/* This function takes in istream of DataPoints and k and returns a vector
 * with the k number of data points with the highest priority value in
 * descending order. It uses the PQArray data structure to keep a leaderboard
 * of the values from the stream that have the highest priority.
 */
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    Vector<DataPoint> final(pq.size(), {"",0});
...
STUDENT_TEST("pqSort: time trial") {
    Vector<int> startSizes = {5000, 10000, 20000, 40000};
    for (int j: startSizes){
        Vector<DataPoint> v;
        fillVector(v, j);
        TIME_OPERATION(j, pqSort(v));
        }
    }

STUDENT_TEST("topK: time trial, k constant") {
...
STUDENT_TEST("topK: time trial, startSize constant") {
    int startSize = 200000;
    int k = 10;
    for (int n = k; n < 10*k; n *= 2) {
        Vector<DataPoint> input;
        fillVector(input, startSize);
        stringstream stream = asStream(input);
